...
Version 1.10 · 2026-09-25 · Status: Phase 1 implemented, deployed
...
| Edit Profile | ✓ | ✓ (a pending or rejected one goes back for review) | ✓ (a pending or rejected one goes back for review) | |
...
| PATCH | /profiles/:id | Founder, Manager, Associate — anyone who works with Profiles; Experts are refused. A Founder's edit changes nothing else. Anyone else editing a **pending or rejected** Profile sends it back to pending and notifies the Founders; editing an **approved** one leaves it approved, because keeping details current must not take a Profile out of use over a corrected phone number. The onboard date and the Manager's share are the Founder's and are ignored from anyone else |
...
**Priority** (P1, P2, P3) is not a field anyone sets: `priorityOf` reads it off
the quadrant a task sits in. Nothing on screen shows it since the quadrants
left the interface, but the rule and its tests stay, because the columns are
still on the row.
...
**Urgency** (`urgencyFromDays`) is how pressing a task is, 0–100, as a share of
...
and 100 on the day — flat while there is room, steep when there is not. Days
are counted in the reader's own days, so "due tomorrow" means their tomorrow.
...
| Tasks | all | **One table**, and nothing else on the page: Owner, Start, End date, **Urgency**, Description, Status — opening with the most urgent first. Every heading sorts, and under every heading is that column's filter — who owns it, starting on or after a day, due on or before a day, a search over the words, and a status (**Active** by default: everything not yet Completed, plus anything completed in the last 7 days, so that finishing a task never makes it vanish under your hand). A past end date is red, and the urgency score reddens as it climbs. A row opens the task in a dialog (§18): owner, the two dates with their own labels, and, behind **More details**, the description, expected deliverable, definition of done and what it waits for — with **Save & create another** for a batch. An end date typed with no start warns on the spot. Status is one dropdown: the owner moves it along (Not Started, In Progress, Blocked — which asks why — and Ready for Review), and whoever gave the task confirms it or sends it back. On a task you gave yourself the last choice reads **Completed**, because there is nobody to review it and ticking it finishes it. Rows are **dragged** by the handle on the left: up and down to set the order a person's tasks are kept in, or onto someone else's row to hand the task to them (the giver only, while it is unfinished and not from a chat). That order is what the table opens in; clicking a column heading sorts by it instead and the handles go quiet until the handle heading is clicked again. A task can also be handed over by changing the Owner on a task you gave |
...
- Unit (`packages/shared`, 1487 tests): `canTransition` against every (role,
...
- API (`apps/api`, 410 tests): transition endpoint returns 403 for wrong role,
