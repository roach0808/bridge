...
import { Box, Button, Card, Checkbox, Collapse, Divider, IconButton, Stack, Tooltip, Typography } from '@mui/material';
import {
...
import type { Filter } from './TodosPage';
import { TaskWhen } from './taskShared';
import { TODO_COLORS, todoText, useRefreshTodos } from './todoShared';
...
/**
 * The board: one panel per person, each in four quadrants — what needs action
 * and what can wait, strategic and not. Tasks are dragged between quadrants and
 * between people; the order is saved for everyone who sees the panel.
 */
      ...
      <Stack spacing={2}>
        {panels.map((panel) => (
          <PersonPanel
            ...
            filter={filter}
            dragging={dragging}
            onNewTask={() => onNewTask(panel.person)}
            onReopen={onReopen}
            onOpen={onOpen}
          />
        ))}
...
function Quadrant({
  ...
  const open = tasks.filter((t) => isActiveTodo(t.status)).length;
      ...
      {tasks.length === 0 ? (
        ...
        <SortableContext items={tasks.map((t) => t.id)} strategy={verticalListSortingStrategy}>
          <Stack divider={<Divider />}>
            {tasks.map((t) => (
              <SortableTask key={t.id} todo={t} draggable={arrange || canHandOn(t, me.id)} onReopen={() => onReopen(t)} onOpen={() => onOpen(t)} />
            ))}
          </Stack>
        </SortableContext>
      )}
...
function SortableTask({ todo, draggable, onReopen, onOpen }: { todo: TodoDTO; draggable: boolean; onReopen: () => void; onOpen: () => void }) {
...
function TaskRow({
  ...
  const canDelete = iGave && (isActiveTodo(t.status) || t.status === 'completed');
  ...
  return (
    ...
    >
      <Stack direction="row" spacing={0.5} alignItems="center" sx={{ minHeight: 40 }}>
        {handle}
        <Tooltip title={isActiveTodo(t.status) ? (mine ? 'Tick when you have finished it' : `Waiting for ${t.assignee.nickname}`) : `Done ${t.doneAt ? relativeTime(t.doneAt) : ''}`}>
          <span>
            <Checkbox
              size="small"
        ...
        <Box sx={{ minWidth: 0, flex: 1, cursor: 'pointer' }} onClick={onOpen} role="button" aria-label={`Open “${text}”`}>
          <Typography
            variant="body2"
            noWrap
            title={text}
